...
Concerto is built on the principle of **"Orchestration as Code"** -- making AI agent coordination a first-class programming concern rather than an afterthought bolted onto existing languages through libraries.

### Core Beliefs

1. **AI harnesses deserve their own language.** General-purpose languages and library-based approaches (LangChain, CrewAI) force developers to work around fundamental mismatches between language abstractions and AI orchestration patterns. Concerto removes this friction by making agents, tools, memory, and structured output native constructs.
...
4. **Orchestration patterns should be declarative.** Multi-agent pipelines, branching logic, retry strategies, and error recovery should be expressed as clear, readable code -- not buried in callback hell or framework configuration files.
...
- Provide a purpose-built language for designing complex AI agent harnesses
- Enable type-safe LLM interactions with compile-time validation where possible
- Support multi-agent orchestration with first-class pipeline constructs
...
1. **Complex agent harnesses** -- Multi-step workflows where agents collaborate, share memory, and produce structured outputs
2. **Structured output extraction** -- Parsing LLM responses into typed schemas with validation and retry logic
3. **Tool-augmented LLM workflows** -- Defining tools that agents can invoke, with parameter schemas auto-generated from type signatures
...
6. **Agent testing and simulation** -- Writing test harnesses that validate agent behavior with mock LLM responses
...
3. The **IR** (`.conc-ir`) is a JSON-based intermediate representation containing instructions, type info, agent/tool definitions, and source maps
...
2. Defines a `Greeter` agent with a system prompt
