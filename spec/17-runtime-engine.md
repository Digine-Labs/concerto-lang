...
The Concerto runtime is a stack-based virtual machine that executes IR generated by the compiler. It manages the execution loop, LLM connections, model lifecycle, tool dispatch, memory hashmaps, the emit channel system, schema validation, error handling, and async execution.
...
- Register models, tools, schemas, connections, hashmaps, pipelines
...
loop {
    ...
    match instruction.op {
        ...
        CALL_MODEL => { dispatch_model_call(instruction) }
...
Manages model instances, their configurations, and lifecycle.

**Responsibilities:**
- Initialize models from IR model definitions
- Bind models to their connections
- Attach tools to models
- Bind memory hashmaps
- Execute model methods (route to Connection Manager)
...
**Responsibilities:**
- Execute async model calls without blocking the main loop
...
let models = runtime.inspect_models();
```
...
| `tokens_by_model(name)` | Tokens per model |
